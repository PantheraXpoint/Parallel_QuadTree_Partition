...
def intersectLineJob(items) -> bool:
    """
    ...
    polygon, idx, p0, p1 = items
    e0 = polygon.points[idx]
    e1 = polygon.points[(idx + 1) % len(polygon.points)]
    if polygon.doIntersect(p0, p1, e0, e1):
        return True
    return False

def touchIntersectJob(items) -> bool:
    """
    Check if there exist any intersection between polygon and a line segment
    p0: starting point of line segment
    p1: ending point of line segment
    """
    polygon, i, p0, p1 = items

    e0 = polygon.points[i]
    e1 = polygon.points[(i + 1) % len(polygon.points)]
    # if self.__onSegment(p0,e0,p1) and self.__onSegment(p0,e1,p1):
    #     return True
    if polygon.doTouch(p0, p1, e0, e1) or polygon.doIntersect(p0, p1, e0, e1):
        return True
    return False
...
class Polygon:
    ...
    def onSegment(self, p: Point, q: Point, r: Point) -> bool:
        if (
    ...
    def doIntersect(self, p1: Point, q1: Point, p2: Point, q2: Point) -> bool:
        """
    ...
    def doTouch(self, p1: Point, q1: Point, p2: Point, q2: Point) -> int:
        """
        The function that returns true if
        the line segment 'p1q1' and 'p2q2' intersect.
        """
    ...
    def intersectLine1(self, p0: Point, p1: Point) -> bool:
        """
        Returns true if the line segment intersects with
        the edges of self
        p0: starting point of line segment
        ...
        processPool = Pool(cpus)
        ret = any(processPool.imap_unordered(intersectLineJob, zip(repeat(self), idxs, repeat(p0),repeat(p1)), chunksize=cpus))
        processPool.close()
        return ret
    ...
    def intersectLine(self, p0: Point, p1: Point) -> bool:
        ...
        for idx in range(len(self.points)):
            ...
            if self.doIntersect(p0, p1, e0, e1):
                return True
        return False
    ...
    def touchIntersect(self, p0: Point, p1: Point) -> bool:
        ...
        for i in range(len(self.points)):
            ...
            # if self.__onSegment(p0,e0,p1) and self.__onSegment(p0,e1,p1):
            #     return True
            if self.doTouch(p0, p1, e0, e1) or self.doIntersect(p0, p1, e0, e1):
                return True
        return False
    def touchIntersect1(self, p0: Point, p1: Point) -> bool:
        """
        Check if there exist any intersection between polygon and a line segment
        p0: starting point of line segment
        p1: ending point of line segment
        ...
        idxs = range(len(self.points))
        processPool = Pool(cpus)
        ret = any(processPool.imap_unordered(touchIntersectJob, zip(repeat(self), idxs, repeat(p0),repeat(p1)), chunksize=cpus))
        processPool.close()
        return ret

    def containsPoint(self, p: Point) -> bool:
        # if polygon contains the input point
        right = self.touchIntersect1(p, Point(self.maxLon, p.lat))
        left = self.touchIntersect1(Point(-1, p.lat),p)
        up = self.touchIntersect1(p, Point(p.lon, self.maxLat))
        down = self.touchIntersect1(p, Point(p.lon, -1))
...
begin = time.time()
testPoly = Polygon("U:\FINAL\PARALLEL\Testcases\DienBien.geojson","file")
